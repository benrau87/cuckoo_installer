...
echo -e "${YELLOW}Installation complete, login as $name and open the terminal. Change to the $name home directory and execute the ./firstrun.sh script to finish setup. In $name home folder you will find the start_cuckoo.sh script and the rooter.sh scripts. You will need to run rooter.sh as sudo before launching Cuckoo. To get started as fast as possible you will need to create a virtualbox vm and name it ${RED}cuckoo1${NC}.${YELLOW} On the Windows VM install the windows_exes that can be found under the tools folder. Name the snapshot ${RED}vmcloak${YELLOW}. Alternatively you can create the VM with the vmcloak.sh script provided in your home directory. This will require you have a local copy of the Windows ISO you wish to use. You can then launch cuckoo_start.sh and navigate to $HOSTNAME:8000 or https://$HOSTNAME if Nginx was installed.${NC}"
